...
def handle_news():
    ...
    db = DatabaseHandler()
    db.insert_news(current_date, city, text)
...
def handle_private_ad():
    ...
    days_left = (expiration_date_dt - today).days
    record = f"PRIVATE_AD|{text}|{expiration_date_dt.strftime('%Y-%m-%d')}|{days_left} days left"
    write_to_file(record)
    db = DatabaseHandler()
    db.insert_private_ad(text, expiration_date_dt.strftime('%Y-%m-%d'), days_left)
...
def handle_custom_record():
    ...
    write_to_file(record)
    db = DatabaseHandler()
    db.insert_custom(current_date, title, description)
...
class FileProcessor:
    def __init__(self, path=None):
        self.path = path or "default_folder/records.txt"
    ...
    def remove_file(self):
        try:
            os.remove(self.path)
            print(f"File {self.path} has been successfully processed and removed.")
        except Exception as e:
            print(f"Error removing file {self.path}: {e}")
    ...
    def process_record(self, item):
        ...
        if record_type == "news":
            ...
            try:
                ...
                write_to_file(record)
                db.insert_private_ad(text, expiration_date.strftime('%Y-%m-%d'), days_left)
            except ValueError:
...
class DatabaseHandler:
    def __init__(self, db_path="news_records.db"):
    ...
    def create_tables(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS custom_records (
                ...
                title TEXT,
                description TEXT,
                UNIQUE(date, title, description)
            )
        ''')
        self.conn.commit()

    def insert_news(self, date, city, text):
        try:
            self.cursor.execute('''
                INSERT INTO news (date, city, text) VALUES (?, ?, ?)
            ''', (date, city, text))
            self.conn.commit()
            print("Inserted news into DB")
    ...
    def insert_private_ad(self, text, expiration_date, days_left):
        try:
            ...
            self.conn.commit()
            print("Inserted private ad into DB")
        except sqlite3.IntegrityError:
            print("Duplicate private ad found, skipping insert.")

    def insert_custom(self, date, title, description):
        try:
            self.cursor.execute('''
                INSERT INTO custom_records (date, title, description) VALUES (?, ?, ?)
            ''', (date, title, description))
            self.conn.commit()
            print("Inserted custom record into DB")
        except sqlite3.IntegrityError:
            print("Duplicate custom record found, skipping insert.")
...
if __name__ == "__main__":
    # Process TXT
    if input("Process TXT? (y/n): ").strip().lower() == "y":
        file_path = input("Path to TXT (Enter for default): ").strip()
        FileProcessor(file_path if file_path else None).process_file()

    # Process JSON
    if input("Process JSON? (y/n): ").strip().lower() == "y":
        json_path = input("Path to JSON (Enter for default): ").strip()
        JSONProcessor(json_path if json_path else None).process_json()

    # Process XML
    if input("Process XML? (y/n): ").strip().lower() == "y":
        xml_path = input("Path to XML (Enter for default): ").strip()
